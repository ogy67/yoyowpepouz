...
public class infoMenuManager : MonoBehaviour
{
    ...
    private void fillInfos()
    {
        ...
        {
            difficultyInfo_text.text = traductionManager.getInstance().getTradTutoDifficulty(difficultyBar.prev_level_difficulty);
        }
    ...
    {
        if (managerNiveau.gameOn || notifDefiDone.isAnimatingNotif || scrollLevels.isScrollingList())
            return;
