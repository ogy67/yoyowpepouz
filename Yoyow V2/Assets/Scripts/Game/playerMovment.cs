...
public class playerMovment : MonoBehaviour
{
    ...
    public float DEG_SPEED_CONST = 200; // 180 // 150 avant
    ...
    public const float factorTranslate = 2f; // vitesse de translation
    ...
    */
    ...
    public bool Collide(Collision2D collision) // return true si collision ne conduit pas a la mort
    {
        ...
        bool colIsBehindPlayer = colBehindPlayer(releaseVelocity, collision.contacts[0].point, collision.contacts[0].normal);
            ...
            )*/)
        {
            ...
            /* souris le fait de elle même aussi
            bool avoidObstacle = comportement == typeBehaviour.tuto ?
                collision.gameObject.tag == "Obstacle" ? false : true
                :
                collision.gameObject.tag == "Obstacle"
                &&
                !boostIngameManager._bouclierOn
                /*&& // si use ghost, ne rien faire du tout, ignorer collision
                !boostIngameManager.getInstance().canUse_ghost(collision.gameObject)
                && // si use ressort, rebondir
                !(checkRessort && boostIngameManager.getInstance().canUse_ressort())
                ? false : true;*/
            ...
            if (!avoidObstacle)
            {
                ...
                else if (comportement == typeBehaviour.tuto)
                {
                    forceDie(collision);
                }
                // ne survit pas
                 ...
                 || bounceOnObstacleAfterWin)
            {
                ...
                /*if (!finish) // si pas finit lvl
                {
                    computeNorm(collision);
                    if (hasReleased && !TutoManager.tutoOn)
                    {
                        defiManager.getInstance().defiIsIncremented(typeDefi.bounce, 1);
                        gameManager.getInstance().eventIngame(typeEffectColor.bounce);
                    }
