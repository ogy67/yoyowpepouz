...
public class traductionManager : MonoBehaviour
{
    ...
    private string[] testDifficultyTrad_fr = new string[] { "FACILE", "MOYEN", "DIFFICILE", "EXPERT", "LEGENDE" };
    private string[] testDifficultyTrad_en = new string[] { "EASY", "MEDIUM", "DIFFICULT", "EXPERT", "LEGEND" };
    ...
    private Dictionary<typeTuto_tuto, string> infoTuto_tuto_fr = new Dictionary<typeTuto_tuto, string>
    {
        {typeTuto_tuto.push, "Envoies le palet dans la cible" },
        {typeTuto_tuto.slow, "Appuies sur<color=\"#FFB800\"> B</color> pour activer la precision de tir" },
        {typeTuto_tuto.checkP, "Restes appuyé sur<color=\"#FFB800\"> A</color>" +
            " pour t'accrocher au <color=\"#FF4F50\">Cercle Rouge</color>" },
        {typeTuto_tuto.switchR, "Glisses<color=\"#FFB800\"> A</color> pour inverser le sens de rotation" },
    };
    private Dictionary<typeTuto_tuto, string> infoTuto_tuto_en = new Dictionary<typeTuto_tuto, string>
    {
        {typeTuto_tuto.push, "Throw the puck into the target" },
        {typeTuto_tuto.slow, "Click on<color=\"#FFB800\"> B</color> to activate puck throw precision" },
        {typeTuto_tuto.checkP, "Keep pressing<color=\"#FFB800\"> A</color>" +
            " to hang on the <color=\"#FF4F50\">Red Circle</color>" },
        {typeTuto_tuto.switchR, "Slide<color=\"#FFB800\"> A</color> to reverse the direction of rotation"},
    };

    public string getTradInfoTuto_tuto(typeTuto_tuto type)
    {
        return currentLanguage == defautLanguage ? infoTuto_tuto_en[type] : infoTuto_tuto_fr[type];
    }
