...
public class offreQuotidienne : MonoBehaviour
{
    ...
    */
    ...
    public long[] getTimeLeft_newOffer()
    {
        ...
        /*int nbHour = 0;
        while (nbHour < nbHourPastLong)
            nbHour++;
        int nbMinute = 0;
        while (nbMinute < nbMinutePastLong)
            nbMinute++;*/



        //float ecart = (ElapsedTime() - prevTime) + delayTimerAdded_sec * 1000;



        //int nbHourPast = (int)(ecart / oneHourTime);
        //int nbMinutePast = (int)((ecart - (nbHourPast * oneHourTime)) / oneMinuteTime);



        //Debug.Log("<color=green> getTimeLeft_newOffer(),</color> heure : " + (23 - nbHourPastLong) +", minutes : "+ (59 - nbMinutePastLong));
        // int[] : heure, minutes
        return new long[]
        {
            23 - nbHourPastLong,
