...
public class chechpointEnd : MonoBehaviour
{
    ...
    {
        give = true;
        yield return new WaitForSeconds(TutoManager.tutoOn ? 0.6f : timeWaitGiveReward);
        ...
        if (!TutoManager.tutoOn)
        {
            ...
            else
            {
                ...
                // 1 yow d'office
                int currLevelDifficulty = menuLevelUpInfinis.currentDifficultyLevel;
                while(currLevelDifficulty > 0)
                {
                    currLevelDifficulty--;
                    rewards.Add(typeReward_game.yow);
                }
        ...
        {
            // animMonnaie("monneyComeGame"); : TUTO le fait lui meme
            popYowGemManager.getInstance().popRec_game(rewards, myPosInUI);
        }
        give = false;
    }
