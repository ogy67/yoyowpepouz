...
public class gameManager : MonoBehaviour
{
    ...
    */
    ...
    public void pickUpYowInfinis() // appelé quand player ramasse YOW sur level, mode infinis
    {   
        StartCoroutine(giveMonney(true, 1, false, true));
        popYowGemManager.popCoinGame();
        popYowGemManager.getInstance().openCompteurYow_inGame();
        StartCoroutine(waitPickMonneyEnd());
    }
    ...
    public void giveRecompense(recompense type, bool arriveeReward)
    {
        ...
        switch (type)
        {
            ...
            case recompense.GEMME:
                StartCoroutine(giveGemme_inGame());
                break;
            case recompense.VIE:
                nbVies++;
                updateLife();
    ...
    private IEnumerator giveGemme_inGame()
    {
        gemmeCompteurAnim.Play("diamondComeGame");
        yield return new WaitForSeconds(3/6f);
        // anim gemme incremente
        gemmesManager.getInstance().addGemme(); // fait l'animation d'incrémentation
    ...
    public void setInitValues()
    {
        ...
        nbVies = expertMode.modeExpertOn && !infinisManager.infinis_Mode_On ? 1 : VIE;
        updateLife();
    }
    ...
    public float timePlayerPop_progBarSet = 0.2f;
    ...
    public IEnumerator loadGame()
    {
        ...
        // ICI, on test si doit ouvrir un tuto aventure
        bool openTuto = menuTuto.tryOpenTuto(SaveSystem.nb_couronne, managerNiveau.currentNiveau);
    ...
    /*public void win_online(bool lastLap, bool winWithBoost)
    {
        StartCoroutine(win_online_routine(lastLap, winWithBoost));
    }

    // est appelé quand player passe dans ligne arrivée en allant en avant
    private IEnumerator win_online_routine(bool lastLap, bool winWithBoost)
    {
        // fonction appelé pour sûr quand passe dans arrivée
        ...
        // _si c'était le dernier lap
        if (lastLap)
        {
            // ___ si gagne avec un boost ( Bombe / PierreT ) => ne pas réactiver player ( rien faire de + )
            // ___ sinon -> stopper player ( pMov.arrivee() )
            if (!winWithBoost)
            {
            ...
            yield return new WaitForSeconds(0.5f);
            gameOn = false;
            // on stop player ( le faire disparaître )
            player.GetComponent<playerMovment>().stop();
        }
        // _sinon
        else
        {
            // ___ si passe avec un boost ( Bombe / PierreT ) => réactiver player ( pMov.spawn() )
            if (winWithBoost)
            {
                yield return new WaitForSeconds(0.5f);
                player.GetComponent<playerMovment>().activate();
            }
            // ___ sinon -> ne pas stopper player ( rien faire de + )
        }
    }*/
        ...
        if (!infinisManager.infinis_Mode_On && playerMovment.comportement != typeBehaviour.online)
        {
            ...
            else
            {
                if (!menuGameOver.hasRevived && currentPack.proposerVieSupp()
                    && launcher.getInstance().adIsLoaded(launcher.nameAd[typeAd.revive_ad]))
